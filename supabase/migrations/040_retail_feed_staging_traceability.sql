-- Sprint 3A.3 — Canonical funnel traceability + source lifecycle gate (additive).
--
-- Purpose:
--   Wire the durable intake worker into the canonical staging RPC. This migration is
--   forward-only and additive (it does NOT edit 039 in place):
--     1. adds keyed traceability columns on retail_price_observations
--        (intake_job_id, intake_draft_ref) so a staged observation can be followed
--        back to the durable intake job + durable draft that produced it (ADR-B);
--     2. re-defines stage_retail_feed_observation to (a) accept the two new trailing
--        traceability params (DEFAULT null, so existing 26-arg callers still resolve),
--        (b) enforce the ADR-D source lifecycle gate (suspended/revoked/retired/
--        inactive sources cannot stage), and (c) persist the traceability columns.
--   No shopper-facing product write, no verification, no publication — staging stays
--   review_status='pending' only. NOT auto-applied to any hosted project.
--
-- Safety: additive columns (nullable); the function stays SECURITY DEFINER with a
--   locked search_path, schema-qualified, parameterized, service_role-only EXECUTE.
--   Identity/replay authority is UNCHANGED (md5 observation_hash + unique-pending index).

-- ══ 1. Additive traceability columns ══════════════════════════════════════════
alter table public.retail_price_observations
  add column if not exists intake_job_id    uuid,
  add column if not exists intake_draft_ref text;

comment on column public.retail_price_observations.intake_job_id is
  'Durable intake job (retail_intake_jobs.id) that produced this staged observation, when it '
  'entered through the canonical funnel. NULL for the legacy CSV compatibility path.';
comment on column public.retail_price_observations.intake_draft_ref is
  'Stable durable draft reference (retail_intake_job_drafts.draft_ref) for this observation.';

create index if not exists rpo_intake_job_idx
  on public.retail_price_observations (intake_job_id)
  where intake_job_id is not null;

-- ══ 2. Re-define the staging RPC (adds lifecycle gate + traceability) ══════════
-- Drop the 039 signature first (adding parameters changes the function identity).
...
create function public.stage_retail_feed_observation(
  p_actor                 uuid,
  p_source_id             uuid,
  ...
  p_intake_job_id         uuid default null,   -- NEW: durable job traceability (ADR-B)
  p_intake_draft_ref      text default null    -- NEW: durable draft traceability (ADR-B)
)
returns table (
...
begin
  ...
  -- Rights boundary (unchanged): reference-only / needs-legal-review / rejected / expired cannot stage.
  ...
  -- ADR-D source lifecycle gate: suspended / revoked / retired / inactive cannot stage.
  if coalesce(src.is_active, true) = false
     or coalesce(src.lifecycle_state, 'discovered') in ('suspended','revoked','retired') then
    return query select 'rejected', null::uuid, null::uuid, null::text, 'not_applicable', null::text,
      format('source lifecycle blocks staging (is_active=%s, lifecycle_state=%s)', src.is_active, src.lifecycle_state);
    return;
  end if;
  ...
  -- Governed branch-mapping resolution (internal ids ONLY from an approved mapping) — unchanged.
  if p_branch_external_id is not null then
    ...
    if v_any_branch > 0 and not exists (
      ...
      if v_usable = 1 then
        ...
        v_map_outcome := 'mapping_invalid';
      end if;
    end if;
  elsif p_external_branch_name is not null then
    v_map_outcome := 'ambiguous';
  end if;
  ...
  -- Deterministic identity + content hashes (AUTHORITY — unchanged from 039).
  ...
  -- Idempotency: identical replay vs conflict vs new — unchanged.
  ...
  -- Upsert source listing identity — unchanged.
  ...
  -- Insert the PENDING observation (never verified, never published) + traceability.
  insert into public.retail_price_observations (
    ...
    feed_parse_warnings, staged_actor, intake_job_id, intake_draft_ref
  ) values (
    ...
    p_parse_warnings, p_actor, p_intake_job_id, p_intake_draft_ref
  ...
  values (p_actor, 'stage_retail_feed_observation', 'retail_price_observations', v_obs_id,
    jsonb_build_object('source_id', p_source_id, 'observation_identity', v_identity, 'review_status', 'pending',
      ...
      'intake_job_id', p_intake_job_id, 'intake_draft_ref', p_intake_draft_ref));
...
comment on function public.stage_retail_feed_observation is
  'Fail-closed pending-review staging bridge (Sprint 2L-B + 3A.3). Stages into retail_source_listings '
  '+ retail_price_observations at review_status=pending ONLY; resolves internal ids solely from an '
  'approved governed mapping; enforces source rights + lifecycle gates; records intake job/draft '
  'traceability; idempotent on a deterministic identity; never verifies, never publishes.';

revoke all on function public.stage_retail_feed_observation(uuid,uuid,text,text,text,text,text,text,text,text,text,bigint,bigint,boolean,text,text,text,text,text,text,text,timestamptz,text,integer,text,jsonb,uuid,text) from public;
revoke all on function public.stage_retail_feed_observation(uuid,uuid,text,text,text,text,text,text,text,text,text,bigint,bigint,boolean,text,text,text,text,text,text,text,timestamptz,text,integer,text,jsonb,uuid,text) from anon;
revoke all on function public.stage_retail_feed_observation(uuid,uuid,text,text,text,text,text,text,text,text,text,bigint,bigint,boolean,text,text,text,text,text,text,text,timestamptz,text,integer,text,jsonb,uuid,text) from authenticated;
grant execute on function public.stage_retail_feed_observation(uuid,uuid,text,text,text,text,text,text,text,text,text,bigint,bigint,boolean,text,text,text,text,text,text,text,timestamptz,text,integer,text,jsonb,uuid,text) to service_role;
