...
function normalizePreviewFloor(value: unknown): string | null {
...
function uniquePreviewFloors(nodes: Array<{ floor?: string | null }>): string[] {
  const seen = new Set<string>();

  for (const node of nodes) {
    const floor = normalizePreviewFloor(node.floor);
...
router.get("/jobs/:jobId/floorplans/:floorPlanId/preview-model", async (req: Request, res: Response) => {
  ...
  const jobId = req.params.jobId as string;
  const floorPlanId = req.params.floorPlanId as string;

  const { data: job, error: jobErr } = await supabase
    .from("map_factory_jobs")
    .select("id, mall_id, status, stage, readiness_score, notes, created_at, updated_at")
    .eq("id", jobId)
    .single();
  ...
  const { data: floorplan, error: floorplanErr } = await supabase
    ...
    .select("id, job_id, mall_id, floor_label, version, layout_json, svg_output, status, notes, created_at")
    .eq("id", floorPlanId)
    .eq("job_id", jobId)
    .single();

  if (floorplanErr || !floorplan) {
    return res.status(404).json({ ok: false, error: "Floorplan not found for this job" });
  }

  if (floorplan.mall_id !== job.mall_id) {
    return res.status(409).json({ ok: false, error: "Floorplan mall_id does not match job mall_id" });
  }

  const [{ data: nodeRows, error: nodeErr }, { data: edgeRows, error: edgeErr }] = await Promise.all([
    supabase
      .from("mall_nodes")
      .select("id, name, type, floor, x_coordinate, y_coordinate, linked_shop_id, source")
      .eq("mall_id", job.mall_id),
  ...
  if (nodeErr) {
    return res.status(500).json({ ok: false, error: "Failed to load mall_nodes", detail: nodeErr.message });
  }

  if (edgeErr) {
    return res.status(500).json({ ok: false, error: "Failed to load mall_edges", detail: edgeErr.message });
  }
  ...
  const nodes = ((nodeRows ?? []) as Array<{
    ...
    ...node,
    floor: normalizePreviewFloor(node.floor),
  }));
  ...
  const floors = uniquePreviewFloors(nodes);
  const activeFloor = normalizePreviewFloor(floorplan.floor_label) ?? floors[0] ?? null;

  const floorNodeIds = new Set(
    nodes
      .filter((node) => !activeFloor || normalizePreviewFloor(node.floor) === activeFloor)
      .map((node) => node.id),
  );

  const filteredNodes = activeFloor
    ? nodes.filter((node) => normalizePreviewFloor(node.floor) === activeFloor)
    : nodes;
  ...
  res.json({
    ok: true,
    preview: {
      mode: "admin_draft_floorplan_preview",
      warning: "Admin/dev preview only. Do not expose draft/stub floorplans to shopper navigation.",
      job,
    },
    model: {
      mall_id: job.mall_id,
      ...
      floorplan: {
        id: floorplan.id,
        job_id: floorplan.job_id,
        floor_label: floorplan.floor_label,
        version: floorplan.version,
        svg_output: floorplan.svg_output,
        layout_json: floorplan.layout_json,
        status: floorplan.status,
        notes: floorplan.notes,
        created_at: floorplan.created_at,
      },
      source: "admin_map_factory_draft_preview",
      reality_label: `ADMIN PREVIEW · ${floorplan.status} floorplan · not shopper-visible`,
      counts: {
