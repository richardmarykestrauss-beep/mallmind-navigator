...
describe("NavigationSession — the visitor journey as a pure reducer", () => {
  it("session creation: a trusted anchor, no destination, destination_selection", () => {
    const s = createNavigationSession(grm.id, anchorFor(grm, "grm-entrance-4", "qr"));
    ...
    expect(s.anchor).toMatchObject({ nodeId: "grm-entrance-4", label: "Entrance 4", source: "qr" });
  });

  it("selecting a destination calculates the route → route_ready (topology only for an unscaled mall)", () => {
    const s = run(grm, createNavigationSession(grm.id, anchorFor(grm, "grm-entrance-4")), { type: "select_destination", destination: poi(grm, "Woolworths") });
  ...
  it("route start → navigating at step 1; Next advances and records completed steps; the last step is arrival", () => {
    let s = run(grm, createNavigationSession(grm.id, anchorFor(grm, "grm-entrance-4")),
  ...
  it("start_navigation / next / previous are ignored outside their states (no route → no walking)", () => {
    const idle = createNavigationSession(grm.id, anchorFor(grm, "grm-entrance-4"));
  ...
  it("re-anchor mid-route: destination preserved, trusted start replaced, route recalculated, steps reset, still navigating", () => {
    const before = run(reds, createNavigationSession(reds.id, anchorFor(reds, "entrance-main")),
    ...
    expect(after.anchor).toMatchObject({ nodeId: "entrance-2", label: "Entrance 2", source: "qr" });
  ...
  it("re-anchor before starting keeps route_ready; re-anchor with no destination just moves the start", () => {
    const ready = run(reds, createNavigationSession(reds.id, anchorFor(reds, "entrance-main")), { type: "select_destination", destination: poi(reds, "Game") });
    ...
    const idle = navigationReducer(reds, createNavigationSession(reds.id, anchorFor(reds, "entrance-main")), { type: "reanchor", anchor: anchorFor(reds, "entrance-2") });
  ...
  it("unroutable: a destination without an honest path → unroutable with the engine's reason; re-anchoring can recover", () => {
    ...
    const s = run(disconnected, createNavigationSession(reds.id, anchorFor(reds, "entrance-2")), { type: "select_destination", destination: poi(reds, "Clicks") });
  ...
  it("restart returns to route_ready at step 1; clear_destination returns to destination_selection", () => {
    const walked = run(grm, createNavigationSession(grm.id, anchorFor(grm, "grm-entrance-4")),
  ...
  it("metric preservation vs unscaled suppression is decided by the route, not the session", () => {
    const metric = run(reds, createNavigationSession(reds.id, anchorFor(reds, "entrance-main")), { type: "select_destination", destination: poi(reds, "Clicks") });
    ...
    const unscaled = run(menlyn, createNavigationSession(menlyn.id, anchorFor(menlyn, "menlyn-lf-entrance-13")), { type: "select_destination", destination: poi(menlyn, "Clicks") });
...
describe("navigationSessionStore — remembers the destination for deep-link re-entry only", () => {
  const base = () => run(grm, createNavigationSession(grm.id, anchorFor(grm, "grm-entrance-4", "qr")), { type: "select_destination", destination: poi(grm, "Pick n Pay") }, { type: "start_navigation" });
  ...
  it("a session without a destination clears the record; corrupt records are ignored", () => {
    localStorage.clear();
    persistNavigationSession(base(), 1_000);
    persistNavigationSession(createNavigationSession(grm.id, anchorFor(grm, "grm-entrance-4")), 1_001);
