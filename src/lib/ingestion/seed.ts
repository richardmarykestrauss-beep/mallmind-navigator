...
import type {
  ...
  StoreOfferObservation, SourceSnapshot, IngestionRun, ReviewQueueItem, SourcePolicy,
} from "./model";
...
export function buildSeedDatabase(now: number = Date.now()): IngestionDatabase {
  ...
  const runs: IngestionRun[] = [
    { id: "run_seed", sourceType: "manual_admin", filename: null, startedAt: ago(2 * DAY), completedAt: ago(2 * DAY - HOUR), status: "completed", totalRows: offers.length, acceptedRows: offers.filter((o) => o.published).length, rejectedRows: 0, warningRows: 1, initiatedBy: "seed", notes: "Curated demonstration seed for Data Ingestion Sprint 1." },
  ];

  const reviewQueue: ReviewQueueItem[] = [
    { id: "rq_user_hisense43", entityType: "product_offer", entityId: "offer_user_hisense43", reason: "User-submitted price — verify against a retailer source before publishing.", severity: "warning", status: "pending", createdAt: ago(2 * HOUR), reviewedAt: null, reviewedBy: null, decisionNotes: null },
    { id: "rq_wool_samsung43", entityType: "product_offer", entityId: "offer_wool_samsung43", reason: "Manual entry older than the freshness window — re-verify.", severity: "info", status: "pending", createdAt: ago(2 * DAY), reviewedAt: null, reviewedBy: null, decisionNotes: null },
  ];
  ...
  return {
    meta: { isPrototype: true, dataOrigin: "curated_demonstration", label: "Curated demonstration data", createdAt: nowIso, version: 1 },
    malls: [mall], retailers, stores, products, offers, observations, snapshots, runs, reviewQueue, policies,
  };
}
