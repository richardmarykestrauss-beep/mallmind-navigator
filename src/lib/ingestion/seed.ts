...
export function buildSeedDatabase(now: number = Date.now()): IngestionDatabase {
  ...
  const snapshots: SourceSnapshot[] = [
    ...
    // A registry-only snapshot awaiting review (mall directory evidence).
    { id: "snap_mallreds_dir", sourceUrl: "https://www.mallatreds.co.za/stores", sourceType: "mall_directory", retrievedAt: ago(10 * DAY), contentHash: "sha256:snap-directory", evidenceExcerpt: "Store directory lists Game (G12), Checkers (L05), Pick n Pay (G20), Woolworths (G02).", evidenceMetadata: { note: "Admin-captured evidence (prototype — no live page capture stored)." }, parserVersion: "ingest-rc1", ingestionRunId: "run_snapshot_dir", status: "captured", retailerId: null, mallId: mall.id, reviewStatus: "staged", reviewedBy: null, notes: "Used to confirm physical store presence only." },
  ];

  const runs: IngestionRun[] = [
    { id: "run_seed", runType: "manual_entry", sourceType: "manual_admin", filename: null, evidenceUrl: null, startedAt: ago(2 * DAY), completedAt: ago(2 * DAY - HOUR), status: "completed", totalRows: offers.length, acceptedRows: offers.filter((o) => o.published).length, rejectedRows: 0, warningRows: 1, conflictsDetected: 1, staleItemsDetected: 1, initiatedBy: "seed", notes: "Curated demonstration seed for Data Ingestion RC1." },
    { id: "run_csv_demo", runType: "manual_csv", sourceType: "csv_import", filename: "checkers-specials-2026-07.csv", evidenceUrl: null, startedAt: ago(20 * HOUR), completedAt: ago(20 * HOUR - 5 * 60000), status: "completed", totalRows: 6, acceptedRows: 4, rejectedRows: 1, warningRows: 1, conflictsDetected: 0, staleItemsDetected: 0, initiatedBy: "admin", notes: "Demo CSV import (prototype — local persisted data)." },
    { id: "run_snapshot_game", runType: "source_snapshot", sourceType: "retailer_product_page", filename: null, evidenceUrl: "https://www.game.co.za/hisense-43a4k", startedAt: ago(3 * HOUR), completedAt: ago(3 * HOUR), status: "completed", totalRows: 1, acceptedRows: 1, rejectedRows: 0, warningRows: 0, conflictsDetected: 0, staleItemsDetected: 0, initiatedBy: "admin", notes: "Manual source snapshot registration." },
    { id: "run_snapshot_dir", runType: "source_snapshot", sourceType: "mall_directory", filename: null, evidenceUrl: "https://www.mallatreds.co.za/stores", startedAt: ago(10 * DAY), completedAt: ago(10 * DAY), status: "needs_review", totalRows: 1, acceptedRows: 0, rejectedRows: 0, warningRows: 0, conflictsDetected: 0, staleItemsDetected: 0, initiatedBy: "admin", notes: "Mall directory snapshot — pending review." },
    { id: "run_agent_reserved", runType: "future_agent_research", sourceType: "aggregator_reference", filename: null, evidenceUrl: null, startedAt: ago(1 * DAY), completedAt: null, status: "staged", totalRows: 0, acceptedRows: 0, rejectedRows: 0, warningRows: 0, conflictsDetected: 0, staleItemsDetected: 0, initiatedBy: "admin", notes: "Reserved: future Paperclip/Perplexity/Claude research import. Not executed in RC1." },
  ];

  // Source registry — public sources MallMind is tracking. RC1 does NO auto-fetching.
  const sources: Source[] = ([
    { id: "src_game", name: "Game — product pages", sourceUrl: "https://www.game.co.za/", sourceType: "retailer_product_page", retailerId: "ret_game", mallId: "mall_reds", status: "approved", riskLevel: "low", legalRiskNote: "Public product pages; observe robots.txt & rate limits. No automated fetching in RC1.", lastCheckedAt: ago(3 * HOUR), ownerNotes: "Primary electronics source for Mall@Reds." },
    { id: "src_checkers_specials", name: "Checkers — weekly specials", sourceUrl: "https://www.checkers.co.za/catalogue", sourceType: "retailer_specials_page", retailerId: "ret_checkers", mallId: "mall_reds", status: "candidate", riskLevel: "medium", legalRiskNote: "Catalogue is time-boxed; verify validUntil. Manual review only.", lastCheckedAt: ago(20 * HOUR), ownerNotes: "Good for catalogue specials." },
    { id: "src_takealot", name: "Takealot — marketplace listings", sourceUrl: "https://www.takealot.com/", sourceType: "marketplace_listing", retailerId: "ret_takealot", mallId: null, status: "needs_review", riskLevel: "medium", legalRiskNote: "Marketplace ToS restrict automated access. Online-only — not a Mall@Reds store.", lastCheckedAt: ago(4 * HOUR), ownerNotes: "Use for online comparison only." },
    { id: "src_pricecheck", name: "PriceCheck — aggregator reference", sourceUrl: "https://www.pricecheck.co.za/", sourceType: "aggregator_reference", retailerId: null, mallId: null, status: "candidate", riskLevel: "medium", legalRiskNote: "Aggregator — benchmark/reference only, never presented as MallMind-verified.", lastCheckedAt: ago(2 * DAY), ownerNotes: "Cross-check benchmark only." },
    { id: "src_login_walled", name: "Example login-walled retailer", sourceUrl: "https://members.example-retailer.co.za/", sourceType: "retailer_product_page", retailerId: null, mallId: null, status: "blocked", riskLevel: "high", legalRiskNote: "Behind a login wall — DO NOT ingest. Blocked per policy.", lastCheckedAt: ago(30 * DAY), ownerNotes: "Blocked: login wall / anti-bot." },
    { id: "src_mallreds_dir", name: "Mall@Reds store directory", sourceUrl: "https://www.mallatreds.co.za/stores", sourceType: "mall_directory", retailerId: null, mallId: "mall_reds", status: "approved", riskLevel: "low", legalRiskNote: "Public mall directory; used for store presence only.", lastCheckedAt: ago(10 * DAY), ownerNotes: "Source of physical store mappings." },
