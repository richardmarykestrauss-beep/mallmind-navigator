 ...
 *
 * A lightweight admin/control-plane view over the local ("Prototype — local
 * persisted data") ingestion store. This is NOT autonomous scraping: it is a
 * read-and-decide surface that shows where every offer came from, what is staged
 * vs approved, which prices conflict or have gone stale, and the audit trail of
 * ingestion runs + review decisions.
 *
 * Designed so future Paperclip / Perplexity / Claude research outputs can later
 * be pasted or imported as source snapshots + staged offers into these same
 * sections — no schema change required.
 */

import { useMemo, useState } from "react";
import { Link } from "react-router-dom";
import {
  ArrowLeft, LayoutDashboard, Globe, Boxes, CheckCircle2, AlertTriangle, Clock,
  History, ClipboardCheck, StickyNote, ShieldCheck, ExternalLink,
} from "lucide-react";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { cn } from "@/lib/utils";
import { loadDatabase, saveDatabase, decideSourceStatus, decideOfferReview } from "@/lib/ingestion/store";
...
import {
  SOURCE_STATUS_META, RUN_TYPE_META, RUN_STATUS_META, sourceTypeLabel,
} from "@/components/ingestion/ingestionMeta";
import { TrustBadge, AvailabilityStatusBadge, FreshnessBadge, ToneBadge } from "@/components/ingestion/badges";
import type { IngestionDatabase, SourceRegistryStatus } from "@/lib/ingestion/model";
  ...
  return (
    <div className="mx-auto min-h-screen max-w-6xl px-4 py-6 md:px-6">
      ...
      <div className="space-y-5">
        {/* 1 — Source Registry */}
        <SectionCard icon={<Globe className="h-4 w-4 text-primary" />} title="Source Registry" count={db.sources.length}>
            ...
            <table className="w-full min-w-[920px] text-sm">
              <thead className="text-left text-[11px] uppercase tracking-wide text-muted-foreground">
                <tr className="border-b border-border/60">
                  ...
                  <th className="px-4 py-2">Status</th><th className="px-4 py-2">Legal / risk</th><th className="px-4 py-2">Last checked</th>
                  <th className="px-4 py-2">Owner notes</th><th className="px-4 py-2">Set status</th>
                </tr>
              </thead>
              <tbody>
                {db.sources.length === 0 && <tr><td colSpan={8}><Empty>No sources registered. Add one from the Sources tab in Data Ingestion.</Empty></td></tr>}
                {db.sources.map((s) => (
        ...
        {/* 2 — Staged Offers */}
        <SectionCard icon={<Boxes className="h-4 w-4 text-amber-300" />} title="Staged Offers" count={staged.length}>
          <div className="overflow-x-auto">
            <table className="w-full min-w-[820px] text-sm">
              <thead className="text-left text-[11px] uppercase tracking-wide text-muted-foreground">
                <tr className="border-b border-border/60">
                  <th className="px-4 py-2">Product / Retailer</th><th className="px-4 py-2">Price</th><th className="px-4 py-2">Trust</th>
                  <th className="px-4 py-2">Availability</th><th className="px-4 py-2">Freshness</th><th className="px-4 py-2">Flags</th><th className="px-4 py-2">Action</th>
                </tr>
              </thead>
              <tbody>
                ...
                {staged.map((o) => (
                  <tr key={o.id} className="border-b border-border/40">
                    ...
                    <td className="px-4 py-2"><Button variant="neon" size="sm" onClick={() => approveOffer(o.id)}><CheckCircle2 className="h-3.5 w-3.5" /> Approve</Button></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {/* 3 — Approved Offers */}
        <SectionCard icon={<CheckCircle2 className="h-4 w-4 text-secondary" />} title="Approved Offers" count={approved.length}>
          <div className="overflow-x-auto">
            <table className="w-full min-w-[820px] text-sm">
              <thead className="text-left text-[11px] uppercase tracking-wide text-muted-foreground">
                <tr className="border-b border-border/60">
                  <th className="px-4 py-2">Product / Retailer</th><th className="px-4 py-2">Price</th><th className="px-4 py-2">Trust</th>
                  <th className="px-4 py-2">Availability</th><th className="px-4 py-2">Freshness</th><th className="px-4 py-2">Source</th>
                </tr>
              </thead>
              <tbody>
                {approved.length === 0 && <tr><td colSpan={6}><Empty>No approved offers yet.</Empty></td></tr>}
                {approved.map((o) => (
                  <tr key={o.id} className="border-b border-border/40">
                    <td className="px-4 py-2">{productName(o.productId)}<span className="block text-[11px] text-muted-foreground">{retailerName(o.retailerId)}</span></td>
                    <td className="px-4 py-2 font-semibold">R{o.currentPrice.toLocaleString("en-ZA")}</td>
        ...
        {/* 4 — Conflict / Stale Alerts */}
        <SectionCard icon={<AlertTriangle className="h-4 w-4 text-red-300" />} title="Conflict / Stale Alerts" count={conflicts.length + stale.length}>
        ...
        {/* 5 — Ingestion Runs */}
        <SectionCard icon={<History className="h-4 w-4 text-primary" />} title="Ingestion Runs" count={db.runs.length}>
          <div className="overflow-x-auto">
            <table className="w-full min-w-[900px] text-sm">
              <thead className="text-left text-[11px] uppercase tracking-wide text-muted-foreground">
                <tr className="border-b border-border/60">
                  <th className="px-4 py-2">Run type</th><th className="px-4 py-2">Status</th><th className="px-4 py-2">Rows found</th>
                  <th className="px-4 py-2">Rows approved</th><th className="px-4 py-2">Conflicts</th><th className="px-4 py-2">Created</th><th className="px-4 py-2">Evidence</th>
                </tr>
              </thead>
              <tbody>
                {runs.length === 0 && <tr><td colSpan={7}><Empty>No ingestion runs recorded.</Empty></td></tr>}
                {runs.map((r) => {
                  ...
                  return (
                    <tr key={r.id} className="border-b border-border/40">
                      ...
                      <td className="px-4 py-2">{r.acceptedRows}</td>
        ...
        {/* 6 — Review Queue */}
        <SectionCard icon={<ClipboardCheck className="h-4 w-4 text-amber-300" />} title="Review Queue" count={queue.length}>
          ...
          <p className="px-4 py-2 text-[11px] text-muted-foreground">Decide items in the <Link to="/admin/data-ingestion" className="text-primary underline">Review queue</Link>.</p>
        </SectionCard>

        {/* 7 — Decision Notes */}
        <SectionCard icon={<StickyNote className="h-4 w-4 text-primary" />} title="Decision Notes" count={decisions.length}>
          <div className="divide-y divide-border/40">
